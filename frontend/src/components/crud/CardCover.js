...
function getCardCoverMessage(templateName, offFunc, handlerMap, messageData) {
  ...
  switch (templateName) {
    ...
    case "fileTooBig":
      let filesizeFormatted = false;
      if (messageData && Object.keys(messageData).length > 0 && 'filesize' in messageData) {
        filesizeFormatted = h.bytesToSize(messageData.filesize);
      }
      return (
        <div className="cover_message">
          <p className="color red">
            <span className="bigtext-1 far fa-flushed"></span>
          </p>

          <p>
            {filesizeFormatted ? (
              <span>Whoa that's a big image! It is {filesizeFormatted}! Please keep it under {h.bytesToSize(cnst.MAX_IMAGE_SIZE, "mb")}.</span>
            ) : (
              <span>Whoa that's a big image! Please keep it under {h.bytesToSize(cnst.MAX_IMAGE_SIZE, "mb")}.</span>
            )}
    ...
    case "invalidForm":
      return (
        <div className="cover_message">
          ...
          <p>
            <button onClick={offFunc}>Sorry about that</button>
          </p>
        </div>
      )
    default:
      return (
        <div className="cover_message">
          <p className="color red">
            <span className="bigtext-1 far fa-bug"></span>
          </p>

          <p><span>This shouldn't happen. It's a bug! Let the developer know!</span></p>
...
class CardCover extends PureComponent {
  constructor(props) {
    ...
    this.behaviorMap = {
      ...
      fileTooBig: { intangible: false, spotlight: false },
  ...
  render() {
    ...
    return (
      <div className={"cover light drag_and_drop" +
        ...
        {getCardCoverMessage(msgType, this.props.off, handlerMap, msgData)}
      </div>
    )
  }
}
